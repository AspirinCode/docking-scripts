...
def main(sys):
    ...
    for type in types:
        ...
        for state in states:
            ...
            file='%s/ranked-%s-pscreen-state%s.targ-allligand%s.out'% (type, sys,
                    int(state), type) 
            if os.path.exists(file):
                ...
                import pdb
                pdb.set_trace()
            file=open('%s/%s-state%s/ranked-%s-pscreen-state%s.targ-allligand%s.out' %
            ...
            file='%s/ranked-%s-pscreen-state%s.targ-ligand%s-decoys.out'% (type, sys, int(state), type) 
            if os.path.exists(file):
                os.system('cp %s %s/%s-state%s/' % (file, type, sys, n))
            else:
                print "no file"
                import pdb
                pdb.set_trace()
            file=open('%s/%s-state%s/ranked-%s-pscreen-state%s.targ-ligand%s-decoys.out' % (type, sys, n, sys, int(state), type))
            decoys=dict()
            for line in file.readlines():
                name=line.split()[0]
            ...
            for x in sorted_all:
                if count < num:
                    ...
                    break
            numpy.savetxt('top_2pertarg.txt', selection, fmt='%s')
            os.system('sort -rnk2 unsorted.tmp > %s/%s-state%s/new-fmt-targ-%s.out' %
                    (type, sys, n, type))
            n+=1
