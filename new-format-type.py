    ...
    for state in states:
        ...
        for type in types:
            ...
            if not os.path.exists('eval-types/%s-state%s/' % ( sys, n)):
                os.mkdir('eval-types/%s-state%s/' % (sys, n))
            ...
            if os.path.exists(file):
                ...
                import pdb
                pdb.set_trace()
            file=open('eval-types/%s-state%s/ranked-%s-aln-state%s.cent-ligand%s.out' %
        ...
        for x in sorted_antagonist:
            ofile.write('0\t%s\n' % x[1])
        ofile.close()
        os.system('sort -rnk2 unsorted.tmp > eval-types/%s-state%s/new-fmt-eval-types.out' %
                (sys, n))
        n+=1
