...
def margins(a):
    ...
    for k in ranged:
        marg = numpy.apply_over_axes(numpy.sum, a, [j for j in ranged if j != k])
...
def expected_freq(observed):
    ...
    Examples
    --------
    >>> observed = numpy.array([[10, 10, 20],[20, 20, 20]])
    ...
    observed = numpy.asarray(observed, dtype=numpy.float64)
    ...
    d = observed.ndim
    expected = reduce(numpy.multiply, margsums) / observed.sum() ** (d - 1)
    return expected
...
def chi2_contingency(observed, correction=True):
    ...
    if numpy.any(expected == 0):
        ...
        zeropos = list(numpy.where(expected == 0)[0])
    ...
    if dof == 0:
        ...
        if dof == 1 and correction:
            ...
            chi2 = ((numpy.abs(observed - expected) - 0.5) ** 2 / expected).sum()
        else:
